...
describe('FvThirdPartyAccountCard', () => {
  ...
  it('should render FvThirdPartyAccountCard', () => {
    ...
    expect(
      wrapper.find('[data-testid="thirdPartyAccountAvatar"]').exists()
    ).toBe(true)
    expect(wrapper.find('[data-testid="thirdPartyAccountName"]').exists()).toBe(
      true
    )
    expect(
      wrapper.find('[data-testid="thirdPartyAccountAddress"]').exists()
    ).toBe(true)
    expect(wrapper.find('[data-testid="thirdPartyAccountCity"]').exists()).toBe(
      true
    )
    expect(
      wrapper.find('[data-testid="thirdPartyAccountCountry"]').exists()
    ).toBe(true)
    expect(
      wrapper.find('[data-testid="thirdPartyAccountLegalStructure"]').exists()
    ).toBe(true)
  })
  ...
  it('should return the name of third party account', () => {
    const wrapper = factory()
    const thirdPartyAccountName = wrapper.find(
      '[data-testid="thirdPartyAccountName"]'
    )
    expect(thirdPartyAccountName.text()).toMatch('plop')
  })
  it('should return the adress of third party account', () => {
    const wrapper = factory()
    const thirdPartyAccountAddress = wrapper.find(
      '[data-testid="thirdPartyAccountAddress"]'
    )
    expect(thirdPartyAccountAddress.text()).toMatch('13 rue des plops')
  })
  it('should return the city of third party account', () => {
    const wrapper = factory()
    const thirdPartyAccountCity = wrapper.find(
      '[data-testid="thirdPartyAccountCity"]'
    )
    expect(thirdPartyAccountCity.text()).toEqual('bourg la reine 14775')
  })
  it('should return the country of third party account', () => {
    const wrapper = factory()
    const thirdPartyAccountCountry = wrapper.find(
      '[data-testid="thirdPartyAccountCountry"]'
    )
    expect(thirdPartyAccountCountry.text()).toEqual('France (FR)')
  })
  it('should return the legal structure of third party account', () => {
    const wrapper = factory()
    const thirdPartyAccountLegalStructure = wrapper.find(
      '[data-testid="thirdPartyAccountLegalStructure"]'
    )
    expect(thirdPartyAccountLegalStructure.text()).toEqual(
